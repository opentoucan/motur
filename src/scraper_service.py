...
class ScraperService:
    ...
    async def scrape_link(self, url: str) -> Optional[str]:
      ...
      if self.settings.scraper.disable_sandbox:
        options.add_argument("--no-sandbox")
      options.binary_location = self.settings.scraper.chrome_binary_location
      ...
      async with Chrome(options=options) as browser:
        ...
        for filename in glob.glob(f"{self.settings.scraper.image_path}/*.png"):
          os.remove(filename)
